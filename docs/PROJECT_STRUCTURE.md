...
│   │
│   ├── guides/                  # 使用指南（面向用户）
│   │   ├── REORDER_BITABLE_GUIDE.md # 多维表格物理排序工具指南
│   │   ├── RECALL_MESSAGES_GUIDE.md  # 消息撤回功能指南
│   │   ├── MESSAGE_INDEX_GUIDE.md    # 消息序号说明
│   │   └── MESSAGE_RECALL_FAQ.md     # 消息撤回常见问题
│   │
│   ├── technical/               # 技术文档（面向开发者）
│   │   ├── ARCHITECTURE.md      # 架构设计
│   │   ├── BITABLE_NOTES.md     # 多维表格开发笔记
...
- **QUICKSTART.md** - 快速开始指南
- **VERSION** - 版本号文件（纯文本，当前 1.8.0）
- **requirements.txt** - Python依赖包列表
- **_todo.md** - 待办事项和已知限制

### cfg/ - 配置文件
所有项目内配置文件集中管理：
- **config.yaml** - 业务配置文件（profiles、bitable_columns、reorderBitable、auto_classify 等，不提交到版本控制）
- **config.yaml.template** - 可提交的业务配置模板
- **wxgzh_list.yaml** - 微信公众号清单（fakeid、名称、更新频率、last_update）
- 飞书密钥、Token、LLM Key 等敏感信息由本机密钥配置加载，不放在仓库内的 `credentials.*` 或 `llm_credentials.*` 文件中。

### src/ - 源代码
所有Python源代码文件 + 公共模块 + 工具脚本：
...
- **reorderMain.py** - 多维表格物理排序主程序，默认预览，`--execute` 执行重排
- **reorderSorter.py** - 多维表格排序规则模块，负责字段排序、日期范围和重复 URL 根记录聚类
- **reorderTreeBuilder.py** - 多维表格父子关系树模块，负责家族树、断链和循环引用处理
...
python src/reorderMain.py --show-records                 # 预览并显示目标顺序明细
...
python -m unittest tests/test_reorder.py  # 测试多维表格物理排序
```
...
| `MESSAGE_RECALL_FAQ.md` | `docs/guides/MESSAGE_RECALL_FAQ.md` |
...
A: 业务配置位于 `cfg/config.yaml`，可提交模板位于 `cfg/config.yaml.template`。敏感凭证由本机密钥配置加载，不放在仓库文件中。

---

## 文件统计

- **源代码**: 多个主程序 + 3个公共模块 + Excel工具脚本
- **配置文件**: 3个（config.yaml、config.yaml.template、wxgzh_list.yaml）在 cfg/ 目录
- **测试脚本**: 多个Python测试文件
- **使用指南**: 多个Markdown文件
- **技术文档**: 多个Markdown文件
- **历史归档**: 5个Markdown文件
...
**总计**: 主要文件按代码、测试、文档、配置、数据和日志目录分类管理
